...
The download comes with everything you need to use this standalone, using calc loops. If you are using the skill system, these are the same calc loops used there and you will have to combine the contents of the loops. Note that this does come with a calc loop for HP restoration, which at the time of posting this is not present in the skill system; included in a subfolder and commented out by default are what you need to replace the HP restoration skills with for compatibility in that department. At some point this calc loop will be merged into the skill system, at which point this paragraph will become obsolete.
